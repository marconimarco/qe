...
const regex = /\{\/\* Composer IBM Quantum \(Mockup\) \*\/\}\s*<div className="bg-slate-950 border border-white\/15 rounded-xl p-4 font-mono text-xs leading-relaxed text-slate-300 shadow-2xl flex flex-col gap-3 relative overflow-hidden">[\s\S]*?<\/div>\s*<\/div>/m;

const newComposer = `{/* Composer IBM Quantum (Mockup) */}
...
if (file.match(regex)) {
  file = file.replace(regex, newComposer);
  console.log("Success patch Composer Visualizer");
} else {
  console.log("Failed patch Composer Visualizer");
}
