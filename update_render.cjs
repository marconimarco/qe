...
const regex = /const parts = text\.split\(\/\(```\[\\s\\S\]\*\?```\)\/g\);[\s\S]*?return \(\s*<div className="flex flex-col gap-2\.5">[\s\S]*?\{parts\.map\(\(part, idx\) => \{[\s\S]*?\}\)\}\s*<\/div>\s*\);\s*\};/m;
...
// replace using regex
if (file.match(/const parts = text\.split\(\/\(```\[\\s\\S\]\*\?```\)\/g\);/)) {
  const parts = file.split("const parts = text.split(/(```[\\s\\S]*?```)/g);");
  const p2 = parts[1].split("  };\n\n  // Current active scenarios")[0];
  file = file.replace("const parts = text.split(/(```[\\s\\S]*?```)/g);" + p2, newRender);
  console.log("Success exact patch render");
} else {
  console.log("Failed to match split");
}
