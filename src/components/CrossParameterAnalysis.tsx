...
export default function CrossParameterAnalysis({ onGeneratePdf, onOpenDominoModal }: CrossParameterAnalysisProps) {
  ...
  return (
    <div className="w-full bg-[#0d0f12] border border-white/10 rounded-3xl p-6 md:p-8 mt-10 shadow-2xl relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-white/10 relative z-10">
        <div>
          ...
          <p className="text-slate-400 text-xs md:text-sm font-light mt-1 max-w-2xl leading-relaxed">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="animate-domino-pulse px-4 py-2.5 rounded-full bg-gradient-to-r from-red-600/30 to-amber-600/30 border border-red-500/60 text-white text-xs font-semibold uppercase tracking-wider flex items-center gap-2 hover:brightness-125 transition-all cursor-pointer shrink-0"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-full bg-white/10 hover:bg-white/15 border border-white/20 text-white text-xs font-mono uppercase tracking-wider flex items-center gap-2 transition-all cursor-pointer shrink-0"
          >
        ...
        {CROSS_PARAMETER_PROBLEMS.map((problem) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-xs font-mono font-bold px-2.5 py-0.5 rounded-md border text-white bg-white/5 border-white/10">
                      {problem.urgenza}
                    </span>
                    {isPrimary && (
                  ...
                  <h4 className="text-lg md:text-xl font-medium text-white tracking-wide flex items-center gap-2">
                    {problem.titolo}
                  </h4>
              ...
              {/* Expanded content */}
              {isExpanded && (
                <div className="px-5 md:px-6 pb-6 pt-2 border-t border-white/5 space-y-6 animate-in fade-in duration-300">
                  {/* Fasi sequenziali */}
                  <div>
                    <h5 className="text-xs font-mono uppercase tracking-widest text-slate-400 mb-3 flex items-center gap-1.5">
                      <Zap className="w-3.5 h-3.5 text-amber-400" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {problem.fasi.map((fase, fIdx) => (
                        <div
                          key={fIdx}
                          className="p-4 rounded-xl bg-black/60 border border-white/10 flex flex-col justify-between space-y-3 relative overflow-hidden"
                        >
                          <div className="space-y-2">
                  ...
                  <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <button
                      onClick={onOpenDominoModal}
                      className="text-xs text-amber-400 hover:text-amber-300 flex items-center gap-1.5 underline decoration-amber-500/50 underline-offset-4 cursor-pointer"
                    >
                    ...
                    <button
                      onClick={onGeneratePdf}
                      className="w-full sm:w-auto px-5 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white text-xs font-semibold tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer"
                    >
                      <FileText className="w-3.5 h-3.5 text-cyan-400" />
